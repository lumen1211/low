...
async def _gql(session: aiohttp.ClientSession, op_name: str, variables: Dict[str, Any], proxy: Optional[str] = None) -> Any:
    body = {
...
async def _discover_campaign(session: aiohttp.ClientSession, proxy: Optional[str]) -> Dict[str, Any]:
    # минимальный дашборд дропсов для текущего пользователя
    return await _gql(session, "ViewerDropsDashboard", {"isLoggedIn": True}, proxy)
